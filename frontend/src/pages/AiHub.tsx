...
const AiHub: React.FC = () => {
  ...
  return (
          ...
          {displayTabs.map((t) => {
            ...
            return (
              <button
                ...
                className="v-option-tile !flex-col !items-start !gap-2"
              ...
              >
                <span
                  ...
                  style={
                    active
                      ? { background: 'var(--gradient-signature)', color: '#fff', boxShadow: 'rgba(108,99,255,0.35) 0 6px 14px -6px' }
                      : { background: 'var(--color-soft-lavender)', color: 'var(--color-campaign-purple)' }
                  }
                >
                  <Icon size={14} />
                </span>
                <span className="block">
                  <span className="block v-ink" style={{ fontSize: 12.5, fontWeight: 500, lineHeight: 1.2 }}>{t.label}</span>
                  <span className="hidden sm:block v-caption v-quiet" style={{ fontSize: 10.5, lineHeight: 1.3, marginTop: 2 }}>{t.desc}</span>
                </span>
              </button>
            );
          })}
    ...
    >
      {/* Supervision warning */}
      <div
        className="rounded-xl px-4 py-3 flex items-start gap-3 v-body"
        style={{ background: 'rgba(255,181,71,0.12)', border: '1px solid rgba(255,181,71,0.35)', color: '#8a5a00', fontSize: 12.5 }}
      >
        <AlertTriangle size={15} className="shrink-0 mt-0.5" />
        <span>
          <strong>AI supervision required.</strong> Outputs are generated by a language model and may be
          inaccurate or miss context — verify before acting on them.
        </span>
      </div>
