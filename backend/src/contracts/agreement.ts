...
export const buildAgreement = (i: AgreementInput): string => {
  ...
  del.push(
    'The Creator will produce and publish the content described in the brief, submit the links on Campgains Hub, and apply reasonable feedback from the Brand before publishing.',
  );
  sections.push(`2. CAMPAIGN & DELIVERABLES\n${del.join('\n')}`);

  const pay: string[] = [
    `Fee: ${payLine}.`,
    "Payment is made through Campgains Hub. The Brand funds the campaign escrow and payouts are released to the Creator's payout account once the deliverables are submitted and approved.",
  ];
  ...
  sections.push(
    `6. TERM & TERMINATION\nThis agreement starts when both parties have accepted it on Campgains Hub${
    ...
    }. Either party may end it earlier with 14 days' written notice through Campgains Hub. Work delivered and approved before termination is paid in full.`,
  );
...
export const amendmentText = (d: { amount: number; currency: string; frequency: string; day?: number | null; ends_at?: string | null; by: 'brand' | 'creator'; date?: Date }) => {
  ...
  }${d.ends_at ? `, running until ${fmtDate(new Date(d.ends_at))}` : ''}. Proposed by the ${d.by === 'creator' ? 'Creator' : 'Brand'} and accepted by the ${d.by === 'creator' ? 'Brand' : 'Creator'} on Campgains Hub; this amendment replaces section 3 (Compensation).`;
};
...
export const buildAddendum = (i: AddendumInput): string => {
  ...
  const sections: string[] = [];
  sections.push(`ADDENDUM — EXTRA WORK\nTo the Collaboration Agreement for "${i.campaign.title}"${accepted}\nBetween ${i.brand.company} (${i.brand.email}) and ${i.creator.name} (${i.creator.email}) · Prepared on Campgains Hub · ${fmtDate(date)}`);
  ...
  const pay: string[] = [`Fee for this extra work: ${payLine}, in addition to the fees in the main agreement.`, "Payment is made through Campgains Hub and released to the Creator's payout account once the deliverables are submitted and approved."];
  if (i.notes && i.notes.trim()) pay.push(`Additional terms: ${i.notes.trim()}`);
  sections.push(`${tasks.length ? 3 : 2}. COMPENSATION\n${pay.join('\n')}`);
  sections.push(`${tasks.length ? 4 : 3}. TERM\nThis addendum starts when both parties have accepted it on Campgains Hub${endsAt ? ` and ends on ${endsAt}` : ' and runs until the deliverables are approved and paid'}. It ends automatically if the main agreement ends.`);
  ...
  sections.push(`${tasks.length ? 6 : 5}. ACCEPTANCE\nThis addendum is accepted digitally on Campgains Hub and recorded against each party's account email with a timestamp. The Creator may accept, decline, or propose different terms; nothing here is binding until both parties have accepted. The main agreement is not changed by declining this addendum.`);
  return sections.join('\n\n');
};
